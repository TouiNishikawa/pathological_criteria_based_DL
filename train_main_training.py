...
if not os.path.exists("./model/main_trained_models"):
  os.makedirs("./model/main_trained_models")

# checkpointの設定
checkpoint = ModelCheckpoint(
                    filepath="./model/main_trained_models/main_trained_model" + "_batch_" + str(batch_size) + "_{epoch:02d}.h5",
                    monitor='val_loss',
...
if not os.path.exists("./log/main_train"):
  os.makedirs("./log/main_train")

loss_history = hist.history["val_loss"]
np_loss_history = np.array(loss_history)
np.savetxt("./log/main_train/loss_history.txt", np_loss_history, delimiter=",")

acc_history = hist.history["val_accuracy"]
np_acc_history = np.array(acc_history)
np.savetxt("./log/main_train/acc_history.txt", np_acc_history, delimiter=",")

import shutil

max_index = np.argmax(acc_history)
best_path = "./model/main_trained_models/main_trained_model" + "_batch_" + str(batch_size) + "_" + "%02d"%int(max_index+1)  + ".h5"
shutil.copyfile(best_path , "./model/best_main_trained_model.h5")
